...
variable (a variable with no such protection). To this end, two interrelated static analysis techniques are designed and
implemented.
...
prior works. One such work is McBride's "Kleisli Arrows of Outrageous Fortune"~\cite{KleisliArrows}. In that paper, an invalid state is unable to be reached as the result of
a similar type-level tag as the \ttt{Sensitivity} tag in the present paper.
However, significantly more machinery is involved in the McBride paper, as
it is necessary to put such a tag on a monadic type. While a monadic type is
used in the present paper (in the form of the \ttt{Cmd} type), this type
does not use a type-level tag. The creation of such machinery is the
main purpose of the McBride paper.
...
features enabled by singletons~\cite{SingletonsPaper} and, in the future, by
...
SpectreGuard is a powerful tool for protecting code against Spectre attacks and,
in the present work, we have given tools which provide additional guarantees
that this tool will function as intended.
