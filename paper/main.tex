...
\lstset{breaklines=true, basicstyle=\ttfamily\tiny, escapeinside={(*@}{@*)}}
...
\begin{abstract}
  ...
  A Spectre mitigation strategy in the form of a mechanism for marking variables as "non-speculative" was developed in
  \cite{SpectreGuard}. In this paper, we develop an embedded domain specific language (EDSL) in Haskell to provide
  support for this feature. The static type system of Haskell is used so that many basic forms of copying
  secret variables to public variables are disallowed. Additionally, a static analysis was developed which
  will detect some cases of secret variables leaking to public variables via control flow constructs.
...
\begin{tabular}{|l|l|}
  ...
  \end{lstlisting}
  & \begin{lstlisting}
  if ((*@$\overline{\ttt{cond}}$@*)) { (*@$\overline{\ttt{t}}$@*) } else { (*@$\overline{\ttt{t}}$@*) }
  \end{lstlisting}\\

  \hline
  \begin{lstlisting}
  for init (\i -> (cond, update, body))
  \end{lstlisting}
  & \begin{lstlisting}
...
of variables, so that a variable does not get assigned to a variable of a different sensitivity. The
logical inference rules corresponding to this check are given in Fig. \ref{fig:SensTypes}.
...
where \ttt{x} is some expression or command in the EDSL. This judgment means
that \ttt{x} has type $\beta$. This is the same syntax used by Haskell, and by extension
the EDSL, to explicitly specify a type in an expression.
...
useful. A conversion from public to secret would be straightforward. A conversion
going the other direction could be provided by an instruction in the EDSL
which might perform a mitigation measure, such as a cache flush. \cite{PLtea-james}
